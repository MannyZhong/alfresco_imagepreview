...
function main()
{
	 ...
	 //rock---start---->
	   var thumbnail = null;
	   if("pdfpreview" == thumbnailName && "application/pdf" == node.getMimetype()) {
		   thumbnail = node;
	   } else {
		   ...
		   // Get the thumbnail
		   thumbnail = node.getThumbnail(thumbnailName);
...
//	   var reference = [ url.templateArgs.store_type, url.templateArgs.store_id ].concat(url.templateArgs.id.split("/"));
//	   var node = search.findNode(pathSegments[2], reference);
//	var fileName = imagePreviewService.getIndentifyByUrl(pathSegments[2]);
	var lastModify = thumbnail.properties["cm:modified"];
	var totalPages = imagePreviewService.getPageNumber(node.id, thumbnail.getNodeRef());
	model.totalPageNumber = totalPages;
}
